...
- [Development guidelines](#development-guidelines)
    - [General RXB Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
RXB Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
RXB Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_rxb
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_rxb --log_level=test_suite
$ valgrind -v --leak-check=full src/rxbd -printtoconsole
...
# A coverage report will now be accessible at `./test_rxb.coverage/index.html`.
```
...
To profile a running rxbd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep rxbd` -- sleep 60
```
...
RXB Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/Heiwabitnull/rxb-core/issues/12691)
...
- ThreadSocketHandler : Sends/Receives data from peers on port 8327.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 8328 for connections and services them.
...
However, in open source software such as RXB Core, where everyone uses
...
pay attention to for reviewers of RXB Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: RXB Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `RXB_FOO_BAR_H`, e.g.

```c++
#ifndef RXB_FOO_BAR_H
#define RXB_FOO_BAR_H
...
#endif // RXB_FOO_BAR_H
```
...
Some of these are maintained by active developers of RXB Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against RXB Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `rxb-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `rxb-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of RXB Core, the wallet was always
...
- For consistency and friendliness to code generation tools, interface method
  ...
  before in [#14635](https://github.com/Heiwabitnull/rxb-core/pull/14635).
