...
`/Applications/Bitcoin-Qt` (on Mac) or `bitcoind`/`rxb-qt` (on
Linux).
...
- #14521 qt, docs: Fix `rxb-qt -version` output formatting (hebasto)
...
- #14092 Dry run `bench_rxb` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_rxb` (MarcoFalke)
