...
RXB_CONFIG_ALL="--disable-dependency-tracking --prefix=$DEPENDS_DIR/$HOST --bindir=$BASE_OUTDIR/bin --libdir=$BASE_OUTDIR/lib"
...
DOCKER_EXEC ../configure --cache-file=config.cache $RXB_CONFIG_ALL $RXB_CONFIG || ( (DOCKER_EXEC cat config.log) && false)
END_FOLD
