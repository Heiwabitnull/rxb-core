...
TEST_RXB_BINARY="src/test/test_rxb"
...
if [[ ! -e ${TEST_RXB_BINARY} ]]; then
    echo "Error: Executable ${TEST_RXB_BINARY} not found. Run \"./configure --enable-lcov\" and compile RXB."
    exit 1
fi
...
if [[ $(get_file_suffix_count gcno) == 0 ]]; then
    echo "Error: Could not find any *.gcno files. The *.gcno files are generated by the compiler. Run \"./configure --enable-lcov\" and re-compile RXB."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_RXB_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
    ...
    rm "${TEST_OUTPUT_TEMPFILE}"
    if [[ $(get_file_suffix_count gcda) == 0 ]]; then
        echo "Error: Running the test suite did not create any *.gcda files. The gcda files are generated when the instrumented test programs are executed. Run \"./configure --enable-lcov\" and re-compile RXB."
        exit 1
    fi
...
echo
echo "RXB coverage test passed: Deterministic coverage across ${N_TEST_RUNS} runs."
exit
