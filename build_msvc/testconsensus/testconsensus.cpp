...
// rxb includes.
#include <..\src\script\rxbconsensus.h>
...
int main()
{
    std::cout << "rxbconsensus version: " << rxbconsensus_version() << std::endl;
    ...
    rxbconsensus_error err;
    auto op0Result = rxbconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, rxbconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
