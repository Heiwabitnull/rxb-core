...
const char * const RXB_CONF_FILENAME = "rxb.conf";
...
{
    // Split section name from key name for keys like "main.foo"
 ...
 * https://github.com/Heiwabit/rxb/pull/16097#issuecomment-514627823
 */
...
const std::set<std::string> ArgsManager::GetUnsuitableSectionOnlyArgs() const
{
    ...
    // RXB has only mainnet, so this should never happen
...
{
    LOCK(cs_args);
    // RXB only supports mainnet
    if (network != CBaseChainParams::MAIN) {
        LogPrintf("Warning: RXB only supports mainnet. Using mainnet.\n");
        m_network = CBaseChainParams::MAIN;
    } else {
        m_network = network;
    }
}
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //rxb-tx using stdin
        std::string val;
...
{
    // Windows: C:\Users\Username\AppData\Roaming\RXB
    // macOS: ~/Library/Application Support/RXB
    // Unix-like: ~/.rxb
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RXB";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/RXB";
#else
    // Unix-like
    return pathRet / ".rxb";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", RXB_CONF_FILENAME);
...
{
    // RXB only has mainnet, so always use default section
    return true;
}
