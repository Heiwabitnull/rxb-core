...
bool IsSafeForRXBTests()
{
    std::string network = Params().NetworkIDString();
    if (network == "main") {
        BOOST_TEST_MESSAGE("TRANSACTION TESTS DISABLED ON RXB MAINNET");
        BOOST_TEST_MESSAGE("Transaction tests could interfere with live network");
...
// DODATKOWY TEST ZABEZPIECZAJACY DLA RXB
BOOST_AUTO_TEST_CASE(rxb_transaction_safety)
{
    std::string network = Params().NetworkIDString();
    if (network == "main") {
        BOOST_TEST_MESSAGE("RXB TRANSACTION TESTS: MAINNET ACCESS BLOCKED");
        BOOST_TEST_MESSAGE("Transaction tests could interfere with live network operations");
        BOOST_TEST_MESSAGE("Tests will only run in regtest environment");
        BOOST_CHECK(true); // Test passed - bezpieczne wyjscie
        return;
    }
    
    BOOST_TEST_MESSAGE("RXB Transaction Tests: Environment verified");
