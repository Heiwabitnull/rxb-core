...
void MinerTestingSetup::TestPackageSelection(const CChainParams& chainparams, const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    tx.vout[1].nValue = 100000000; // 1 BX output
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    // RXB: Initial subsidy is 100 BX instead of 50 BTC
    ...
    // RXB: Halving interval is 195000 blocks instead of 210000
    // Create an actual 194999-long block chain (without valid blocks).
    while (::ChainActive().Tip()->nHeight < 194999) {
    ...
    BOOST_CHECK(pblocktemplate = AssemblerForTest(chainparams).CreateNewBlock(scriptPubKey));
    // Extend to a 195000-long block chain.
    while (::ChainActive().Tip()->nHeight < 195000) {
        CBlockIndex* prev = ::ChainActive().Tip();
        CBlockIndex* next = new CBlockIndex();
        next->phashBlock = new uint256(InsecureRand256());
        ::ChainstateActive().CoinsTip().SetBestBlock(next->GetBlockHash());
