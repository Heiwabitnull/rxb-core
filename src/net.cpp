    ...
    int nBestReachability = -1;
    {
        ...
        for (const auto& entry : mapLocalHost)
        {
            ...
            if (nReachability > nBestReachability || (nReachability == nBestReachability && nScore > nBestScore))
            {
                ...
                nBestReachability = nBestReachability;
                nBestScore = nScore;
            }
        }
    }
...
void CNode::copyStats(CNodeStats &stats, const std::vector<bool> &m_asmap)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (RXB users should be well used to small numbers with many decimal places by now :)
...
static void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "RXB " + FormatFullVersion();
...
bool CConnman::BindListenPort(const CService& addrBind, std::string& strError, NetPermissionFlags permissions)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. %s is probably already running.").translated, addrBind.ToString(), "RXB");
        else
