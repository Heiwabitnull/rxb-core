...
{
    // SECURITY: Check if we are not on RXB mainnet
    if (Params().NetworkIDString() == "main") {
        BOOST_TEST_MESSAGE("PSBT wallet tests disabled on RXB mainnet for safety");
        BOOST_TEST_MESSAGE("These tests could damage the live blockchain and economy");
        return; // Exit test without executing any operations
    }
    ...
    // Add hd seed - used RXB private key with prefix 0xD2
    CKey key = DecodeSecret("842gfABjGV5axYH5pmmEGJQFL8f6uLP5MWcqCDfUGqSkqAdep8t"); // RXB mainnet - klucz prywatny WIF
...
{
    // Ten test jest bezpieczny - nie operuje na blockchainie ani portfelu
