...
bool IsSafeTestEnvironment()
{
    ...
    if (network == "main") {
        BOOST_TEST_MESSAGE("WALLET CRYPTO TESTS DISABLED ON RXB MAINNET");
        BOOST_TEST_MESSAGE("Crypto tests could compromise wallet security");
...
{
    // WERYFIKACJA BEZPIECZEŃSTWA RXB
...
BOOST_AUTO_TEST_CASE(rxb_crypto_safety) {
    std::string network = Params().NetworkIDString();
    if (network == "main") {
        BOOST_TEST_MESSAGE("RXB CRYPTO TESTS: MAINNET ACCESS BLOCKED");
        BOOST_TEST_MESSAGE("Crypto operations could expose wallet secrets");
        BOOST_TEST_MESSAGE("Tests will only run in regtest environment");
        BOOST_CHECK(true); // Test passed - safe exit
        return;
    }
    
    BOOST_TEST_MESSAGE("RXB Crypto Tests: Environment verified");
