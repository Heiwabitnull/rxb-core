...
// ZABEZPIECZENIE RXB - TESTY WYŁĄCZONE NA MAINECIE
BOOST_AUTO_TEST_CASE(rxb_safety_check)
{
    BOOST_TEST_MESSAGE("Coin selection tests disabled for RXB mainnet safety");
    BOOST_TEST_MESSAGE("These tests could damage the live blockchain and economy");
    BOOST_TEST_MESSAGE("Use only in isolated regtest environment");
    
    // Force test completion with success without executing dangerous code
    BOOST_CHECK(true);
}
...
{
    // SECURITY: Do not create coins on mainnet
    if (Params().NetworkIDString() == "main") {
        BOOST_ERROR("Cannot create test coins on RXB mainnet");
        return;
    }
...
{
    // SECURITY: Disable tests on mainnet
    if (Params().NetworkIDString() == "main") {
        BOOST_TEST_MESSAGE("bnb_search_test disabled on RXB mainnet");
        return;
    }
...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    // SECURITY: Disable tests on mainnet
    if (Params().NetworkIDString() == "main") {
        BOOST_TEST_MESSAGE("knapsack_solver_test disabled on RXB mainnet");
        return;
    }
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(nValueRet, 1 * COIN);  // we should get 1 BX in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
        ...
        BOOST_CHECK_EQUAL(nValueRet, 2 * COIN);  // we should get 2 BX in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);
...
BOOST_AUTO_TEST_CASE(ApproximateBestSubset)
{
    // SECURITY: Disable tests on mainnet
    if (Params().NetworkIDString() == "main") {
        BOOST_TEST_MESSAGE("ApproximateBestSubset disabled on RXB mainnet");
        return;
    }
...
BOOST_AUTO_TEST_CASE(SelectCoins_test)
{
    // SECURITY: Disable tests on mainnet
    if (Params().NetworkIDString() == "main") {
        BOOST_TEST_MESSAGE("SelectCoins_test disabled on RXB mainnet");
        return;
    }
