...
// ZABEZPIECZENIE RXB - SPRAWDZENIE ŚRODOWISKA
bool IsSafeTestEnvironment()
{
    std::string network = Params().NetworkIDString();
    if (network == "main") {
        BOOST_TEST_MESSAGE("WALLET TESTS DISABLED ON RXB MAINNET");
        BOOST_TEST_MESSAGE("Wallet tests could compromise mainnet security");
...
{
    if (!IsSafeTestEnvironment()) return;
...
BOOST_FIXTURE_TEST_CASE(scan_for_wallet_transactions, TestChain100Setup)
{
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(wallet.GetBalance().m_mine_immature, 100 * COIN); // RXB: 100 BX nagroda
    }
    ...
    // after.
    {
        ...
        BOOST_CHECK_EQUAL(response.write(),
            strprintf("[{\"success\":false,\"error\":{\"code\":-1,\"message\":\"Rescan failed for key with creation "
                      ...
                      "by pruning or data corruption (see rxbd log for details) and could be dealt with by "
...
BOOST_FIXTURE_TEST_CASE(coin_mark_dirty_immature_credit, TestChain100Setup)
{
    ...
    BOOST_CHECK_EQUAL(wtx.GetImmatureCredit(), 100*COIN); // RXB: 100 BX nagroda
}

static int64_t AddTx(CWallet& wallet, uint32_t lockTime, int64_t mockTime, int64_t blockTime)
{
    if (!IsSafeTestEnvironment()) return 0;
...
BOOST_FIXTURE_TEST_CASE(ListCoins, ListCoinsTestingSetup)
{
    ...
    BOOST_CHECK_EQUAL(100 * COIN, wallet->GetAvailableBalance()); // RXB: 100 BX nagroda
...
// DODATKOWY TEST ZABEZPIECZAJĄCY DLA RXB
BOOST_AUTO_TEST_CASE(rxb_wallet_safety_check)
{
    std::string network = Params().NetworkIDString();
    if (network == "main") {
        BOOST_TEST_MESSAGE("RXB WALLET TESTS: MAINNET ACCESS BLOCKED");
        BOOST_TEST_MESSAGE("Wallet tests could create real transactions on mainnet");
        BOOST_TEST_MESSAGE("Tests will only run in regtest environment");
        BOOST_CHECK(true); // Test passed - safe exit
        return;
    }
    
    BOOST_TEST_MESSAGE("RXB Wallet Tests: Environment verified");
    BOOST_TEST_MESSAGE("Running in safe network: " + network);
    BOOST_CHECK(true);
}
