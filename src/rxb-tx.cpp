...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " rxb-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  rxb-tx [options] <hex-tx> [commands]  Update hex-encoded rxb transaction\n" +
            "or:     rxb-tx [options] -create [commands]   Create hex-encoded rxb transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded rxb transaction
